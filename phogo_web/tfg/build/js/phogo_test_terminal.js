...
function normalize(command) {
    console.log("normalize", command);
    let c = command.toLowerCase().split(' ').filter((x) => { return x.length > 0} );
    console.warn(c);
    let args = c.slice(1);
    let cmd = c[0];
    return [cmd, args];
...
function pythonInterpreter(command, term) {
    term.error(command);
}

function mainInterpreter(command, term) {
    let [cmd, args] = normalize(command);
    console.log(command, cmd, args);
    switch (cmd) {
        case 'python':
            term.push(pythonInterpreter, {greetings:"python 3", prompt:">>> "});
            break;
        case 'help':
            if (args.length > 0) {
                term.echo(help(args[0]), this.cols(), 4);
                break;
            }
        default:
            term.echo(help('default', this.cols(), 4));
            break;
    }
}
...
$(function() {
    ...
    document.terminal = $('#terminal').terminal(mainInterpreter, {
